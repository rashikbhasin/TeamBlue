...
class Main(object):
    ...
    while(True):
        ...
        input = int(raw_input())
        if input == 1:
            st1=raw_input("Enter event id :")
            #event.set_event_id(st)
            st2 = raw_input("Enter event name :")
            #event.set_name(st)
            st3 = raw_input("Enter event info :")
            #event.set_info(st)
            st4 = raw_input("Enter event date :(YYYY-MM-DD)")
            #event.set_date(st)
            st5 = raw_input("Enter event venue :")
            #event.set_venue(st)
            st6 = raw_input("Enter event city :")
            #event.set_city(st)
            event=Event(st1,st2,st3,st4,st5,st6)
            manager.add(event)
        elif input == 2:
            ...
            st2 = raw_input("Enter  event name for Update :")
            #event.set_name(st)
            st3 = raw_input("Enter event info for Update:")
            #event.set_info(st)
            st4 = raw_input("Enter event date for Update:(YYYY-MM-DD)")
            #event.set_date(st)
            st5 = raw_input("Enter event venue for Update:")
            #event.set_venue(st)
            st6 = raw_input("Enter event city for Update:")
            #event.set_city(st)
            event = Event(st1,st2,st3,st4,st5,st6)
